...
class _HomeScreenState extends State<HomeScreen> with
  ...
  Future<void> _initialDataLoad() async {
    ...
    if (mounted && companyId != null) {
      ...
      if (mounted && serviceCenters.isNotEmpty) {
        ...
        debugPrint(
            "✅ INITIAL LOAD: Automatically selected Service Center ID: ${_selectedServiceCenter
                ?.id}");
        _fetchDataForUI();
      }
    } else {
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        final NewSerialButton=Provider.of<NewSerialButtonProvider>(context,listen: false);

                        if (_selectedServiceCenter == null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("Please select a service center first.")),
                          );
                          return;
                        }
                        final bool? wasSuccessful = await showDialog<bool?>(
                          context: context,
                          builder: (context) {
                            return NewSerialButtonDialog(serviceCenterModel: _selectedServiceCenter!);
                          },
                        );
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  ...
                                  content: CustomSnackBarWidget(
                                      title: "Error",
                                      message: NewSerialButton.errorMessage??"Failed to create new serial. Please try again."
                                  ),
                                ),
                              );
                            } else {
                    ...
                    GestureDetector(
                      onTap: () async {
                        final nextButton=Provider.of<nextButtonProvider>(context,listen: false);
                        ...
                        final bool success=await nextButton.NextButton(serviceCenterId: serviceCenterId ??"", date: formattedDate);
                        ...
                        if (success && mounted) {
                          debugPrint("✅ Next button action successful. Refreshing list...");
                          await serialListProvider.fetchSerialsButton(serviceCenterId!, formattedDate);
                        } else if (mounted) {

                          debugPrint("❌ Next button action failed.");
                          final errorMessage = nextButtonProvider().errorMessage ?? "An unknown error occurred.";
  ...
  void _showDialogBoxManage(SerialModel serial) {
    ...
    if (serviceCenterId == null || serviceId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Error: ID is missing!")),
      );
      return;
    }
      ...
      builder: (context, serialProvider, child) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final String displayTime = DateFormatter.formatApiTimeToDisplayTime(serial.date);
            return Container(
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                                ...
                                child: Center(child: Text(
                                  ...
                                  fontSize: 12,
                                    color: AppColor.getStatusColor(serial.status)
                                ),
                                )
                                )
                            ),
